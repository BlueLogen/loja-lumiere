...
function StepPagamento({ entrega, items, onBack, subtotal, frete, shipping, onSaveOrder, onGenerateOrder }) {
  ...
  async function irParaMP() {
    ...
    try {
      ...
      if (!res.ok || !data.checkout_url) { setError(data.error || 'Erro ao criar pagamento.'); return }
  ...
  return (
    <div className="checkout-form">
      <div className="checkout-form__section" style={{ textAlign:'center' }}>
        ...
        <button className="btn btn--gold btn--full btn--lg" style={{ marginTop:20, fontSize:15 }}
          onClick={irParaMP} disabled={loading}>
          {loading ? 'Preparando…' : '🔒 Abrir Mercado Pago →'}
        </button>
      </div>
